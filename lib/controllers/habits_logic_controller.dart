...
class HabitsLogicController extends GetxController {
    ...
    if (habit.completeDay.isNotEmpty) {
      for (int i = 0; i < habit.completeDay.length; i++) {
        if (habit.completeDay[i]['finishGoals'] == 0 &&
            habit.completeDay.length > 1) {
          dayOff += 1;
        } else {
          double average = habit.completeDay[i]['finishGoals'] /
  ...
  double getAverageAllHabit() {
    ...
    double averageAllHabit =
        double.parse((result / (boxLength - oneTaskHabit)).toStringAsFixed(2));
    return box.isEmpty
        ? 0.0
        : averageAllHabit.isNaN
            ? 0.0
            : averageAllHabit;
  }
  ...
  double getCompleteDay(Habit habit, int date, int month, int year) {
    ...
    if (habit.completeDay.isNotEmpty) {
      for (int i = 0; i < habit.completeDay.length; i++) {
        if (habit.completeDay[i]['day'] == date &&
            habit.completeDay[i]['month'] == month &&
            habit.completeDay[i]['year'] == year) {
          result = habit.completeDay[i]['finishGoals'] /
              habit.completeDay[i]['goals'];
        }
      }
    }
  ...
  double getAverageDaily(Habit habit) {
    ...
    for (int i = 0; i < habit.completeDay.length; i++) {
      if (habit.completeDay[i]['finishGoals'] == 0) {
        ...
        double average =
            habit.completeDay[i]['finishGoals'] / habit.completeDay[i]['goals'];
        count += average;
      }
    }
    lengthDay = habit.completeDay.length - dayOff;
  ...
  int getTotalPerfectDay(Habit habit) {
    int total = 0;
    for (int i = 0; i < habit.completeDay.length; i++) {
      if (habit.completeDay[i]['finishGoals'] ==
          habit.completeDay[i]['goals']) {
        total += 1;
      }
    }
  ...
  double getAveragePerfectDay(Habit habit) {
    ...
    return double.parse((totalPerfect / (habit.completeDay.length - dayOff))
        .toStringAsFixed(2));
  }
  ...
  double getAverageDailyHabits() {
    ...
    double result = count.isNaN
        ? 0.0
        : ((count / (box.length - oneTaskHabit - dayOff)) * 100);
    return result.isInfinite || result.isNaN ? 0.0 : result;
  }
