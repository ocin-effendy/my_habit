...
class DateController extends GetxController {
  ...
  void setWeekInMonth() {
    for (int i = 0; i < currentMonthList.length; i++) {
      if (i < 7)
        week1.add(currentMonthList[i].day);
      else if (i >= 7 && i < 14)
        week2.add(currentMonthList[i].day);
      else if (i >= 14 && i < 21)
        week3.add(currentMonthList[i].day);
      else
        week4.add(currentMonthList[i].day);
    }
  }
  ...
  String checkDayInWeek(int day) {
    if (day != 0) {
      if (week1.contains(day))
        return "week1";
      else if (week2.contains(day))
        return "week2";
      else if (week3.contains(day))
        return "week3";
      else
        return "week4";
    } else {
  ...
  void updateCurrentMonthList(DateTime time) {
    ...
    positionWeekDays = date_util.DateUtils.weekdays
        .indexOf(date_util.DateUtils.weekdays[currentMonthList[0].weekday - 1]);
    update();
  }
